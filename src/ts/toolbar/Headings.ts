    ...
    public _bindEvent(vditor: IVditor, panelElement: HTMLElement) {
        ...
        for (let i = 0; i < 6; i++) {
            panelElement.children
                .item(i)
                .addEventListener(getEventName(), (event: Event) => {
                    event.preventDefault()
                    if (vditor.currentMode === "wysiwyg") {
                        setHeading(
                            vditor,
