 ...
 data-key="${key}"><span class="vditor-emojis__icon">${emojiValue}</span></button>`
        ...
        panelElement.innerHTML = `<div class="vditor-emojis" style="max-height: ${
            vditor.options.height === "auto"
                ? "auto"
                : (vditor.options.height as number) - 80
            ...
            (event: Event & { target: Element }) => {
                ...
                if (btnElement) {
                    ...
                    if (
                        ...
                        html =
                            tempElement.firstElementChild.firstElementChild
                                .outerHTML + " "
                    ...
                    ;(
                        this.element.lastElementChild as HTMLElement
                    ).style.display = "none"
            ...
            (event: Event & { target: Element }) => {
                ...
                if (btnElement) {
                    this.element.querySelector(
                        ".vditor-emojis__tip"
                    ).innerHTML = btnElement.getAttribute("data-key")
                }
            }
        )
    }
